      ...
      <el-form :label-position="'right'"
        ...
        <div class="base-info">
          <el-form-item label="任务名称："
            required
            prop="name">
            <el-input v-model="applyFrom.name"
          ...
          <el-form-item required
            ...
            label="选择地区：">
            <div class="flex-row">
              <multilevelLinkage :listData.sync="processorList "
                :storeData.sync="regionData">
              </multilevelLinkage>
              <el-form-item style="margin-left: 26px;">
                <el-checkbox v-model="restrictedCity"
          ...
          <el-form-item required
            ...
            label="选择省份：">
            <div class="flex-row">
              <el-select v-model="applyFrom.province"
                multiple
                filterable
                allow-create
          ...
          <el-form-item label="数据日期："
            ...
            <div class="flex-row">
              <el-form-item style="margin-right: 34px;">
                <el-select v-model="applyFrom.extractDateType"
                  @change="changeDate">
                  <el-option v-for="item in extractDateType"
                    :key="item"
                    :value="item.value"
                    :label="item.label"></el-option>
                </el-select>
              </el-form-item>
              <el-form-item class="ml16">
                <div>
                  <el-form-item prop="startDate"
                    class="fl"
                    v-if="applyFrom.extractDateType !== '1'">
                    <el-date-picker v-model="applyFrom.startDate"
                      type="month"
                      :editable="false"
                      :clearable="false"
                      format="yyyy-MM"
                  ...
                  <el-form-item class="fl"
                    prop="endDate"
                    v-if="applyFrom.extractDateType !== '1'">
                    <el-date-picker v-model="applyFrom.endDate"
                      type="month"
                      :editable="false"
                      :clearable="false"
                      format="yyyy-MM"
          ...
          <el-form-item label="客户端："
            required
            prop="clientType">
            <el-checkbox-group v-model="applyFrom.clientType"
              @change="clientFn">
              <el-checkbox v-for="item in clientTypeList"
                :key="item"
                :label="item.label"></el-checkbox>
            </el-checkbox-group>
          </el-form-item>
        </div>
        <el-form-item class="detail-setting"
          label="明细设置：">
          <el-checkbox-group v-model="applyFrom.detailSet">
            <el-checkbox :disabled="item.type === 1 || (item.type === 2 && applyFrom.isUseTime)"
        ...
        <div class="detail-set">
          <el-form-item label="用户信息："
            v-if="applyFrom.detailSet.indexOf('用户信息') > -1"
            required
            prop="userInfo">
            <el-checkbox-group v-model="applyFrom.userInfo">
              <el-checkbox :disabled="item.type === 1 && restrictedCity.length > 0 ? true : false"
          ...
          <el-form-item label="用户类型："
            v-if="applyFrom.detailSet.indexOf('用户类型') > -1"
            required
            prop="dataType">
            <el-checkbox-group v-model="applyFrom.dataType">
              <el-checkbox :disabled="(item.type === 1 && !!isByDay) || (item.name === '流失会员用户' && applyFrom.dataType.indexOf('存留会员用户') >= 0) || (item.name === '存留会员用户' && applyFrom.dataType.indexOf('流失会员用户') >= 0)"
          ...
          <el-form-item label="数据来源："
            v-if="applyFrom.detailSet.indexOf('数据来源') > -1">
            <div>
              <el-checkbox class="all-label"
                v-model="sourceAll"
                label="全部"
                @change="isAllChecked2('sourceAll', 'source', sourceList)"></el-checkbox>
              <el-checkbox-group v-model="applyFrom.source"
                @change="handleChecked('sourceAll', 'source', sourceList)">
                <el-checkbox v-for="item in sourceList"
                  :key="item"
                  :label="item.label"></el-checkbox>
              </el-checkbox-group>
            </div>
          ...
          <el-form-item label="会员类型："
            v-if="applyFrom.detailSet.indexOf('会员类型') > -1">
            <div>
              <el-checkbox class="all-label"
                v-model="vipTypeAll"
                label="全部"
                @change="isAllChecked2('vipTypeAll', 'vipType', vipTypeList)"></el-checkbox>
              <el-checkbox-group v-model="applyFrom.vipType"
                @change="handleChecked('vipTypeAll', 'vipType', vipTypeList)">
                <el-checkbox v-for="item in vipTypeList"
                  :key="item"
                  :label="item.label"></el-checkbox>
              </el-checkbox-group>
            </div>
          ...
          <el-form-item label="用户性别："
            v-if="applyFrom.detailSet.indexOf('性别') > -1">
            <div>
              <el-checkbox class="all-label"
                v-model="sexAll"
                label="全部"
                @change="isAllChecked2('sexAll', 'sex', sexList)"></el-checkbox>
              <el-checkbox-group v-model="applyFrom.sex"
                @change="handleChecked('sexAll', 'sex', sexList)">
                <el-checkbox v-for="item in sexList"
                  :key="item"
                  :label="item.label"></el-checkbox>
              </el-checkbox-group>
            </div>
          ...
          <el-form-item label="用户年龄："
            v-if="applyFrom.detailSet.indexOf('年龄') > -1">
            <div>
              <el-checkbox class="all-label"
                v-model="ageAll"
                label="全部"
                @change="isAllChecked2('ageAll', 'age', ageList)"></el-checkbox>
              <el-checkbox-group v-model="applyFrom.age"
                @change="handleChecked('ageAll', 'age', ageList)">
                <el-checkbox v-for="item in ageList"
                  :key="item"
                  :label="item.label"></el-checkbox>
              </el-checkbox-group>
            </div>
          ...
          <el-form-item label="用户行为："
            v-if="applyFrom.detailSet.indexOf('用户行为') > -1">
            <div>
              <el-select v-model="applyFrom.userActive"
                multiple
                placeholder="请选择">
                <el-option v-for="item in userActiveList"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value">
                </el-option>
          ...
          <el-form-item label="上网方式："
            v-if="applyFrom.detailSet.indexOf('上网方式') > -1">
            <div>
              <el-checkbox class="all-label"
                v-model="netTypeAll"
                label="全部"
                @change="isAllChecked2('netTypeAll', 'netType', netTypeList)"></el-checkbox>
              <el-checkbox-group v-model="applyFrom.netType"
                @change="handleChecked('netTypeAll', 'netType', netTypeList)">
                <el-checkbox v-for="item in netTypeList"
                  :key="item"
                  :label="item.label"></el-checkbox>
              </el-checkbox-group>
            </div>
          ...
          <el-form-item label="使用时长："
            v-if="applyFrom.detailSet.indexOf('使用时长') > -1">
            <el-radio-group v-model="applyFrom.serviceTime"
              @change="getServiceTime">
              <el-radio :disabled="applyFrom.isUseTime"
